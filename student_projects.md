### DLBDSIPWP01 - Einführung in die Programmierung mit Python (Projekt)

---

### Aufgabe 1: PenguEats – Entwicklung eines Fischrestaurants für Pinguine

Idee: Wie kann ein Pinguin Python nutzen, um ein erfolgreiches Fischrestaurant zu führen und zu erweitern?

Aufgabe: Erstelle eine Python-basierte Anwendung, in der Pingu, der Pinguin, ein Fischrestaurant eröffnet und Code
verwendet, um:

- **Inventar zu verwalten** (Fischarten, Menge, Frische)
- **Kundenbestellungen abzuwickeln** (andere Tiere, die das Restaurant besuchen)
- **Gewinne und Ausgaben zu erfassen** (Eisblockmiete, Kosten für Fischlieferanten)
- **Rezepte vorzuschlagen** (basierend auf dem vorhandenen Fischbestand)
- **Kundenpräferenzen zu erlernen** (sei hier kreativ)

---

### Aufgabe 2: Panda Spa – Thermalwasser-Wellnesszentrum

Idee: Wie kann ein Panda Python nutzen, um ein luxuriöses Thermalwasser-Spa im Bambuswald zu betreiben?

Aufgabe: Erstelle eine Python-basierte Anwendung, in der Panda ein gemütliches Spa mit Thermalbädern, Massagen und
Tee-Therapie für Waldtiere eröffnet und Code verwendet, um:

- **Spa-Dienstleistungen zu verwalten**
- **Kundentermine abzuwickeln** (andere Tiere, die das Spa besuchen)
- **Gewinne und Ausgaben zu erfassen** (Warmwasser, Tee und andere Spa-Lieferanten)
- **Kundenpräferenzen zu erlernen und Buchungsvorschläge zu machen** (sei hier kreativ)

---

### Aufgabe 3: Bear HoneyWorks – Die Honigfabrik eines Bären

Idee: Wie kann ein Bär Python nutzen, um seine Honigproduktion zu automatisieren und sein Geschäft auszubauen?

Aufgabe: Erstelle eine Python-basierte Anwendung, in der ein Bär eine Honigfabrik eröffnet und Python verwendet, um:

- **Inventar zu verwalten** (Honigsorten, Menge, Frische)
- **Produktionslinien zu steuern** (welche neue Honigsorte eingeführt werden soll)
- **Honig im gesamten Wald zu verkaufen**
- **Visualisierungen** der verkauften Honigmengen der letzten 3 Monate zu erstellen
- **Klassen für Bienen, Blumen, Gläser und Maschinen zu verwenden**
- **Kundenpräferenzen zu erlernen und Kaufvorschläge zu machen** (sei hier kreativ)

---

### Aufgabe 4: FoxPost – Der Paketlieferdienst des Fuchses

Idee: Wie kann ein Fuchs Python nutzen, um einen effizienten Paketlieferdienst im Wald zu betreiben?

Aufgabe: Erstelle eine Python-basierte Anwendung, in der Felix, der Fuchs, einen Paketlieferdienst gründet und Code
verwendet, um:

- **Pakete zu verwalten** (Größe, Gewicht, Empfänger, Lieferstatus)
- **Lieferrouten zu optimieren** (kürzeste Wege zwischen Tierbehausungen berechnen)
- **Kundenaufträge abzuwickeln** (Abholung und Zustellung für Waldtiere)
- **Gewinne und Ausgaben zu erfassen** (Transportkosten, Lagermiete, Verpackungsmaterial)
- **Lieferzeiten vorherzusagen** (basierend auf Entfernung und Wetterbedingungen)
- **Kundenbewertungen zu sammeln und auszuwerten** (sei hier kreativ)

---

### Aufgabe 5: OwlBooks – Die Bibliothek der weisen Eule

Idee: Wie kann eine Eule Python nutzen, um eine moderne Bibliothek für alle Waldtiere zu verwalten?

Aufgabe: Erstelle eine Python-basierte Anwendung, in der Olivia, die Eule, eine Bibliothek im hohlen Baum eröffnet und
Code verwendet, um:

- **Buchbestände zu verwalten** (Titel, Autor, Genre, Verfügbarkeit)
- **Ausleihen und Rückgaben zu organisieren** (welches Tier hat welches Buch)
- **Mahnungen zu erstellen** (bei überfälligen Büchern)
- **Gewinne und Ausgaben zu erfassen** (Neuanschaffungen, Reparaturen, Mitgliedsbeiträge)
- **Buchempfehlungen zu generieren** (basierend auf dem Lesegeschmack der Tiere)
- **Statistiken zu erstellen** (beliebteste Bücher, aktivste Leser) (sei hier kreativ)

---

### Aufgabe 6: RabbitFarm – Der Gemüsehof des Hasen

Idee: Wie kann ein Hase Python nutzen, um einen nachhaltigen Gemüsehof zu betreiben und die Waldtiere mit frischem
Gemüse zu versorgen?

Aufgabe: Erstelle eine Python-basierte Anwendung, in der Rudi, der Hase, einen Gemüsehof gründet und Code verwendet, um:

- **Anbauflächen zu verwalten** (Gemüsesorten, Pflanzzeit, Erntezeitpunkt)
- **Lagerbestände zu überwachen** (Menge, Frische, Haltbarkeit)
- **Kundenbestellungen abzuwickeln** (Abo-Kisten für Waldtiere)
- **Gewinne und Ausgaben zu erfassen** (Saatgut, Dünger, Wasserkosten)
- **Ernteerträge zu visualisieren** (Diagramme der letzten Saisons)
- **Klassen für Gemüsesorten, Beete, Kunden und Lieferungen zu verwenden**
- **Saisonale Angebote und Rezeptvorschläge zu erstellen** (sei hier kreativ)

---

### Aufgabe 7: WolfLeague – Die Fußballliga des Wolfes

Idee: Wie kann ein Wolf Python nutzen, um eine professionelle Fußballliga für Waldtiere zu organisieren und zu
verwalten?

Aufgabe: Erstelle eine Python-basierte Anwendung, in der Wolfgang, der Wolf, eine Fußballliga gründet und Code
verwendet, um:

- **Teams zu verwalten** (Mannschaftsnamen, Spieler, Positionen, Trikotnummern)
- **Spielpläne zu erstellen** (Hin- und Rückrunden, Spieltermine, Spielorte)
- **Spielergebnisse zu erfassen** (Tore, Gelbe/Rote Karten, Torschützen)
- **Tabellenstand zu berechnen** (Punkte, Tordifferenz, Platzierung)
- **Gewinne und Ausgaben zu erfassen** (Ticketverkäufe, Stadionmiete, Schiedsrichterkosten)
- **Spielerstatistiken zu visualisieren** (Torjäger, beste Verteidiger, Fairplay-Wertung)
- **Klassen für Spieler, Teams, Spiele und Stadien zu verwenden**
- **Spielvorhersagen zu erstellen** (basierend auf bisherigen Ergebnissen) (sei hier kreativ)

---

### Aufgabe 8: DeerFit – Das Fitnessstudio des Hirsches

Idee: Wie kann ein Hirsch Python nutzen, um ein modernes Fitnessstudio für Waldtiere zu betreiben?

Aufgabe: Erstelle eine Python-basierte Anwendung, in der Diego, der Hirsch, ein Fitnessstudio auf der Waldlichtung
eröffnet und Code verwendet, um:

- **Mitgliedschaften zu verwalten** (Tiernamen, Mitgliedschaftstyp, Ablaufdatum)
- **Kurspläne zu erstellen** (Yoga, Krafttraining, Ausdauerlauf, Schwimmkurse)
- **Kursbuchungen abzuwickeln** (Anmeldungen, Wartelisten, Absagen)
- **Trainingsfortschritte zu dokumentieren** (Gewicht, Ausdauer, persönliche Rekorde)
- **Gewinne und Ausgaben zu erfassen** (Mitgliedsbeiträge, Gerätekosten, Trainerhonorare)
- **Fitnessberichte zu visualisieren** (Fortschrittskurven der letzten Monate)
- Klassen für Mitglieder, Kurse, Trainer und Geräte zu verwenden
- Personalisierte Trainingspläne zu empfehlen (basierend auf Zielen und Fortschritt) (sei hier kreativ)

---

### Aufgabe 9: Squirrel Secret Stash – Die Nuss-Zentralbank

Idee: Wie kann ein Eichhörnchen Python nutzen, um seine tausenden Verstecke zu verwalten und für den Winter vorzusorgen?

Aufgabe: Erstelle eine Anwendung, in der "Sammy Squirrel" sein Vorratsnetzwerk digitalisiert und Code verwendet, um:

- **Verstecke zu kartieren** (Koordinaten, Baumart, Erdtiefe).

- **Nuss-Inventar zu führen** (Haselnüsse, Walnüsse, Eicheln inklusive Haltbarkeitsdatum).

- **Diebstahl-Warnungen zu generieren** (Vergleich von Soll- und Ist-Bestand nach Besuchen von frechen Eichelhähern).

- **Zinseszins-Berechnungen** für "geliehene" Nüsse an Nachbarn durchzuführen.

- **Winterprognosen** zu erstellen (reicht der Bestand basierend auf der aktuellen Kältewelle?).

---

### Aufgabe 10: Owl’s Night-Owl Library – Die nächtliche Wissensdatenbank

Idee: Wie nutzt eine weise Eule Python, um die größte Bibliothek des Waldes zu organisieren, die nur nachts geöffnet
hat?

Aufgabe: Programmiere ein Verwaltungssystem für Professor Uhu, um:

- **Bücherkataloge zu verwalten** (Titel, Autor, Kategorie wie "Mäusekunde" oder "Sternnavigation").

- **Leihfristen zu überwachen** (Rückgabe bis zum ersten Sonnenstrahl).

- **Flüster-Modus-Check:** Ein Tool, das die Lautstärke der Besucher (Daten-Input) misst und bei Überschreitung
  Warnungen ausgibt.

- **Lese-Empfehlungen** basierend auf der Flügelspannweite oder dem Jagdrevier des Besuchers zu generieren.

---

### Aufgabe 11: Beaver Builders Inc. – Staudamm-Engineering & Logistik

Idee: Wie kann ein Biber-Team Python einsetzen, um komplexe Wasserbauprojekte effizient zu planen?

Aufgabe: Erstelle ein Projektmanagement-Tool für "Benny Biber", um:

- **Baumaterial zu berechnen** (Anzahl Stämme, Schlamm-Volumen in Litern, Steine).

- **Statik-Simulationen** (vereinfacht): Prüfe, ob die Dammhöhe dem Wasserdruck (Input-Variable) standhält.

- **Arbeitszeiterfassung** für die Biber-Truppe (wer hat wie viele Stunden genagt?).

- **Kosten-Nutzen-Analyse** für neue Bauprojekte im Vergleich zum Fischreichtum im entstehenden Teich.

---

### Aufgabe 12: Fox Express – Der flinke Wald-Kurierdienst

Idee: Wie nutzt ein Fuchs Python, um Pakete und Nachrichten zwischen den Bauen der Tiere zuzustellen?

Aufgabe: Entwickle eine Logistik-Software für "Fiona Fuchs", um:

- **Routenoptimierung** zu betreiben (kürzester Weg zwischen Kaninchenbau und Dachshöhle).

- **Paketstatus zu tracken** (Eingegangen, Unterwegs, Vergraben, Zugestellt).

- **Express-Zuschläge** basierend auf der Gefährlichkeit der Route (z.B. "Wolfsgebiet-Passage") zu berechnen.

- **Empfänger-Präferenzen:** Speichere, welches Tier seine Pakete lieber im Moos versteckt oder direkt persönlich
  annimmt.

---

### Aufgabe 13: Raccoon’s Recycling Garage – Upcycling für Waschbären

Idee: Wie kann ein Waschbär Python nutzen, um aus dem Müll der Menschen wertvolle Schätze für die Waldgemeinschaft zu
machen?

Aufgabe: Programmiere eine Anwendung für "Ronny Raccoon", um:

- **Müll-Inventar zu klassifizieren** (Plastik, Metall, "Glitzerndes", Essensreste).

- **Bauleitfäden zu erstellen:** Welcher Schrott lässt sich zu welchem Werkzeug kombinieren?

- **Tauschgeschäfte zu verwalten** (z.B. 3 glänzende Alufolien gegen 1 alten Tennisschläger).

- **Fundort-Statistik:** Erfasse, an welchen Wochentagen die Mülltonnen im Dorf am besten gefüllt sind.

---

### Aufgabe 14: Chameleon Color Consulting – Die Modeberatung

Idee: Wie nutzt ein Chamäleon Python, um anderen Tieren zu helfen, die perfekte Tarnung oder das beste Party-Outfit zu
finden?

Aufgabe: Erstelle eine Stil-App für "Crispy Chameleon", um:

- **Farbanalysen durchzuführen** (Abgleich von Umgebungsfotos mit der Hautfarbe des Kunden).

- **Tarnungs-Score zu berechnen:** Wie gut ist das Tier in einer bestimmten Umgebung (z.B. Wüste, Dschungel) versteckt?

- **Abonnement-Modelle** für saisonale Farbwechsel (Sommerpelz vs. Winterpelz).

- **Stimmungserkennung:** Ändere die Farbvorschläge basierend auf dem emotionalen Zustand des Tieres.

---

### Aufgabe 15: Elephant Memory Cloud – Der Foto- & Archivdienst

Idee: Da Elefanten niemals vergessen, nutzen sie Python, um das riesige Archiv der afrikanischen Savanne zu verwalten.

Aufgabe: Entwickle ein digitales Gedächtnis für "Ella Elefant", um:

- **Ereignisse zu indexieren** (Jahr, Ort der Wasserstelle, beteiligte Herden).

- **Stammbäume zu visualisieren** (wer ist mit wem verwandt?).

- **Suchfunktion für "Wo ist die nächste Wasserquelle?"** basierend auf historischen Daten aus Trockenzeiten.

- **Erinnerungs-Bot:** Benachrichtige andere Tiere an wichtige Jahrestage oder Migrationsstarts.

**TOPIC 1.:** Memory Management: Reference Counting vs. Cyclic Garbage Collection

---

### Aufgabe 16: Mole’s Metro Map – U-Bahn-Bau unter der Wiese

Idee: Wie plant ein Maulwurf ein effizientes Tunnelnetzwerk für den öffentlichen Tier-Nahverkehr?

Aufgabe: Erstelle ein Planungs-Tool für "Monty Maulwurf", um:

- **Tunnelabschnitte zu erfassen** (Tiefe, Bodenbeschaffenheit: Erde, Sand, Wurzelhindernis).

- **Fahrpläne zu erstellen** (Wann gräbt welche Linie wo?).

- **Ticketpreise zu berechnen** (basierend auf der Anzahl der Erdhügel, die als Haltestellen dienen).

- **Bodenfeuchtigkeits-Sensordaten** zu verarbeiten, um Einsturzgefahren bei Regen zu melden.

---

### Aufgabe 17: Honeybee Smart-Hive – Die effiziente Honigfabrik

Idee: Wie nutzt die Bienenkönigin Python, um die Arbeit von tausenden Arbeiterinnen und die Honigproduktion zu steuern?

Aufgabe: Programmiere ein Dashboard für den Bienenstock, um:

- **Blüten-Reporting:** Welche Blumenwiesen geben aktuell den meisten Nektar (Daten-Eingabe der Späher-Bienen)?

- **Lagerplatz-Optimierung** in den Waben (Honig vs. Pollen vs. Brut).

- **Temperatur-Monitoring:** Berechne, wie viele Bienen fächeln müssen, um den Stock bei 35°C zu halten.

- **Schichtpläne** für Sammlerinnen, Wächterinnen und Putzbienen.

---

### Aufgabe 18: Sloth’s Slow-Motion Hotel – Entspannung extrem

Idee: Wie kann ein Faultier Python nutzen, um ein Luxus-Resort zu führen, in dem Langsamkeit das oberste Ziel ist?

Aufgabe: Erstelle eine Hotel-Software für "Sid Sloth", um:

- **Hängematten-Belegung** zu verwalten (Buchungen dauern mindestens eine Woche).

- **Bewegungs-Tracker:** Kunden bekommen Rabatt, je weniger sie sich pro Tag bewegen (Eingabe der "Schritte" – Ziel:
  nahe Null).

- **Menüplan für Blätter-Feinschmecker** (Reifezeit der Blätter berechnen).

- **Weckruf-Service** (der erst 3 Stunden nach der gewünschten Zeit sanft erfolgt).

---