...
# --- Sidebar: Navigation ---
menu = st.sidebar.radio("Menu", ["Dashboard & State", "Hammock Booking", "Movement Tracker", "Wake-Up Service"])
    ...
    with st.form("booking_form"):
        ...
        if submitted:
            try:
                ...
                st.error(str(e).split("Value error, ")[1] if "Value error" in str(e) else str(e))
...
# --- PAGE 4: WAKE UP ---
    ...
    if st.button("Set Alarm"):
        ...
        # Feedback UI
        st.write(f"Request received for: {wanted_time.strftime('%H:%M')} ({day_label}).")
